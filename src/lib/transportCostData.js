...
function bucket(){return{tripCosts:0,dayTrips:0,sportsTrips:0,boardingTrips:0,otherTrips:0,contractualCosts:0,supplementalFees:0,busGuardianFees:0,staffTransport:0,capacityShortfall:0,otherCharges:0,offsets:0,grossCost:0,totalCost:0,netCost:0}}
function addCost(target,r){
  const amount=Number(r.amount||0);
  if(r.costGroup==="trips"){
    target.tripCosts+=amount;
    if(r.categoryCode==="day_trips")target.dayTrips+=amount;
    else if(r.categoryCode==="sports_trips")target.sportsTrips+=amount;
    else if(r.categoryCode==="boarding_trips")target.boardingTrips+=amount;
    else if(r.categoryCode==="other_trips")target.otherTrips+=amount;
  }else if(r.costGroup==="contractual"){
    target.contractualCosts+=amount;
    if(r.categoryCode==="supplemental_fee")target.supplementalFees+=amount;
    else if(r.categoryCode==="bus_guardian_fees")target.busGuardianFees+=amount;
    else if(r.categoryCode==="staff_transport")target.staffTransport+=amount;
    else if(r.categoryCode==="minimum_capacity_shortfall")target.capacityShortfall+=amount;
