...
export function getAvailableTerms(selectedAcademicYear = "") {
  const matchingRecords = selectedAcademicYear
    ...
    : records;
  return unique(matchingRecords.map((record) => record.term)).sort(
    (a, b) => getTermOrder(a) - getTermOrder(b)
  );
}
...
function resolveMonthInfo(record) {
  ...
  if (dateMatch) {
    ...
    return {
      ...
      label: new Intl.DateTimeFormat("en-GB", { month: "short", year: "2-digit", timeZone: "UTC" }).format(date),
  ...
  if (record.termStart) {
    ...
    if (!Number.isNaN(date.getTime())) {
      ...
      const termMonth = date.getUTCMonth() + 1;
      return {
        key: `${year}-${String(termMonth).padStart(2, "0")}`,
        label: new Intl.DateTimeFormat("en-GB", { month: "short", year: "2-digit", timeZone: "UTC" }).format(date),
        sortValue: year * 100 + termMonth,
      };
    }
  }
  return null;
...
export function getSchoolBreakdown(filteredRecords = []) {
  return groupedBreakdown(filteredRecords, (record) => record.school || "Unspecified School", "school");
}

export function getTermBreakdown(filteredRecords = []) {
  return groupedBreakdown(filteredRecords, (record) => record.term || "Unspecified Term", "term")
    .sort((a, b) => getTermOrder(a.term) - getTermOrder(b.term));
}

export function getProgrammeGroupBreakdown(filteredRecords = []) {
  return groupedBreakdown(filteredRecords, (record) => record.programGroup || "Unspecified Group", "programGroup");
}
...
export function formatCurrency(value) {
  return new Intl.NumberFormat("en-AE", { style: "currency", currency: "AED", maximumFractionDigits: 0 }).format(toNumber(value));
}

export function formatCompactCurrency(value) {
  return new Intl.NumberFormat("en-AE", { style: "currency", currency: "AED", notation: "compact", maximumFractionDigits: 1 }).format(toNumber(value));
}
