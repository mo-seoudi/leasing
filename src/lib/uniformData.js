...
function applyUniformYearBasis(record, yearBasis = "finance") {
  if (yearBasis !== "backToSchool") return record;
  const monthNumber = getMonthNumber(record.month);
  if (monthNumber !== 8) return record;
  return {
    ...record,
    academicYear: getBackToSchoolAcademicYear(record.month) || record.academicYear,
    term: "Term 1",
    termOrder: 1,
  };
}
...
function getMonthOrder(yearBasis = "finance") {
  return yearBasis === "backToSchool" ? BACK_TO_SCHOOL_MONTH_ORDER : FINANCE_MONTH_ORDER;
}

function mapUniformSummaryRow(row) {
  const base = {
  ...
  return [
    {
      ...base,
      id: `${row.academic_year}-${row.month}-${row.school_code || "all"}-${row.scenario || "Actual"}-sales`,
      metric: "Sales",
      metricCode: "sales",
    ...
    {
      ...base,
      id: `${row.academic_year}-${row.month}-${row.school_code || "all"}-${row.scenario || "Actual"}-commission`,
      metric: "Commission",
      metricCode: "commission",
...
export async function fetchUniformRecords() {
  const { data, error } = await supabase
    .from("uniform_dashboard_summary")
    .select(`
      academic_year,
      month,
      term,
...
export function getUniformAcademicYears(records = [], yearBasis = "finance") {
  return unique(records.map((record) => applyUniformYearBasis(record, yearBasis).academicYear)).sort((a, b) => a.localeCompare(b));
}
...
export function filterUniformRecords(records = [], { academicYear = "", school = "", term = "", scenario = "Actual", yearBasis = "finance" } = {}) {
  return records
    .map((record) => applyUniformYearBasis(record, yearBasis))
    .filter((record) => {
...
export function getMonthlyUniformData(records, yearBasis = "finance") {
  ...
  return [...grouped.values()]
    ...
    .sort((a, b) => {
      ...
      if (ay !== 0) return ay;
      return monthOrder.indexOf(a.monthNumber) - monthOrder.indexOf(b.monthNumber);
    });
}

export function getSchoolUniformData(records) {
  const grouped = new Map();
  records.forEach((record) => {
...
export function getTermUniformData(records) {
  const grouped = new Map();
  records.forEach((record) => {
...
export function getUniformSummary(records) {
  let sales = 0;
  let commission = 0;
  records.forEach((record) => {
...
export function getUniformAcademicYearComparison(records) {
  const grouped = new Map();
  records.forEach((record) => {
    const academicYear = record.academicYear;
    if (!academicYear) return;
    const current = grouped.get(academicYear) || { academicYear, sales: 0, commission: 0, months: new Set(), schools: new Set() };
  ...
  return [...grouped.values()]
    .map((item) => ({
      ...
      commissionRate: item.sales > 0 ? (item.commission / item.sales) * 100 : 0,
