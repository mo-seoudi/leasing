...
export async function fetchKitchenRentalRecords(){
 const [{data,error},{data:contractTerms,error:contractError}]=await Promise.all([
  supabase.from("financial_records").select(`id,academic_year,month,scenario,amount,school:schools(code,name),stream:revenue_streams!inner(code,name),metric:revenue_metrics(code,name)`).eq("stream.code",STREAM_CODE).eq("metric.code","rental_fees").eq("scenario","Actual").eq("is_deleted",false).order("academic_year",{ascending:true}),
 ...
 (data||[]).forEach(row=>{
  ...
  if(!years.has(key))years.set(key,{academicYear:row.academic_year||"",school:row.school?.name||schoolCode,schoolCode,contracts:applicableContracts(terms,schoolCode,row.academic_year),months:[]});
  const item=years.get(key),amount=Number(row.amount||0),month=normalizeMonth(row.month);
  item.months.push({id:row.id,academicYear:row.academic_year||"",month,label:month,term:termForMonth(month),revenue:amount});
 });
