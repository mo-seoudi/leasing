...
export default function FixedRevenueComparison({records=[],formatCurrency,formatCompactCurrency,revenueLabel="Revenue",startMonth=9}){
 ...
 return <section className="fixed-revenue-comparison">
  ...
  <section className="comparison-chart-card kitchen-rental-comparison-chart"><div className="comparison-card-heading"><div><h2>{chartTitle}</h2><p>{description}</p></div></div><div className="comparison-chart"><ResponsiveContainer width="100%" height="100%"><BarChart data={rows} margin={{top:18,right:20,left:4,bottom:8}} barCategoryGap="28%"><CartesianGrid stroke="#edf1f5" strokeDasharray="3 5" vertical={false}/><XAxis dataKey="academicYear" axisLine={false} tickLine={false}/><YAxis width={76} tickMargin={8} axisLine={false} tickLine={false} tickFormatter={formatCompactCurrency}/><Tooltip formatter={value=>[formatCurrency(value),revenueLabel]}/><Bar dataKey="revenue" name={revenueLabel} fill="#2f80ed" radius={[8,8,2,2]} maxBarSize={58}/></BarChart></ResponsiveContainer></div></section></div>
 </section>
}
